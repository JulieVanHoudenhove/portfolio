...
const ProjectCard = ({ project, background, id }) => {
    return (
        <Link href={`/projets/${project.slug}`} key={id} className={`projectCard px-16 lg:px-32 py-8 lg:py-16 flex justify-between items-center border-t w-full md:h-[150px] lg:h-[200px] ${background === 'blue-pattern' ? 'bg-blue-pattern text-white border-white' : 'bg-cream text-black border-dark'}`}>
